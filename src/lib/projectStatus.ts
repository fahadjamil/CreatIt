...
export function normalizeProjectStatus(raw: unknown): ProjectStatus {
  ...
  if (value.includes("payment") && (value.includes("due") || value.includes("overdue")))
    return "payment_due";
  if (value.includes("dispute")) return "in_dispute";
  ...
  const canonical = value.replace(/-/g, "_") as ProjectStatus;
  if (canonical in LABELS) return canonical;

  return "draft";
}

export function projectStatusLabel(raw: unknown): string {
 ...
 */
export function projectStatusVariant(raw: unknown): ProjectStatus {
