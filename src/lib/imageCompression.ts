type CompressImageOptions = {
  /** Maximum width in pixels (keeps aspect ratio). */
  maxWidth?: number;
  /** Maximum height in pixels (keeps aspect ratio). */
  maxHeight?: number;
  /**
  ...
  quality?: number;
  /**
   * If the original is PNG, keep PNG to preserve transparency.
   * Defaults to true.
   */
  keepPng?: boolean;
};
...
function canvasToBlob(
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (!blob) reject(new Error("Failed to create compressed blob"));
        else resolve(blob);
      },
      mimeType,
      quality,
...
export async function compressImageFile(
  ...
  if (!file || !isCompressibleImageType(mime)) return file;
